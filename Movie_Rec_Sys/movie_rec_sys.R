...
rating_movieIds <- length(unique(ratings$movieId)) #10325
...
for (c in 1:ncol(binary_ratings2)){
  for (i in 1:ncol(genre_matrix3)){
    result[i,c] <- sum((genre_matrix3[,i]) * (binary_ratings2[,c])) #ratings per genre
  }
}
...
similarity_users <- similarity(rating_mat[1:4, ], 
                               method = "cosine", 
                               which = "users")
...
similarity_items <- similarity(rating_mat[, 1:4], method =
                                 "cosine", which = "items")
...
image(rating_mat, main = "Heatmap of the rating matrix") # hard to read-too many dimensions
image(rating_mat[1:10, 1:15], main = "Heatmap of the first rows and columns")
image(rating_mat[rowCounts(rating_mat) > quantile(rowCounts(rating_mat), 0.99),
                 colCounts(rating_mat) > quantile(colCounts(rating_mat), 0.99)], 
      main = "Heatmap of the top users and movies")


#Normalize the data
rating_mat_norm <- normalize(rating_mat)
image(rating_mat_norm[rowCounts(rating_mat_norm) > quantile(rowCounts(rating_mat_norm), 0.99),
                colCounts(rating_mat_norm) > quantile(colCounts(rating_mat_norm), 0.99)], 
      main = "Heatmap of the top users and movies")

#Create UBFC Recommender Model. UBCF stands for User-Based Collaborative Filtering
recommender_model <- Recommender(rating_mat_norm, 
                                 method = "UBCF", 
